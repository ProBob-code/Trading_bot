...
class NewsSentimentAnalyzer:
    ...
    def get_recent_news(self, limit: int = 20, symbol: Optional[str] = None, market: Optional[str] = None) -> List[Dict]:
        ...
        if market:
            ...
            forex_keywords = ['forex', 'fx', 'dollar', 'euro', 'gbp', 'jpy', 'aud', 'cad', 'currency', 'yield', 'central bank', 'fed', 'interest rate']
            commodity_keywords = ['gold', 'silver', 'copper', 'oil', 'crude', 'wti', 'gas', 'commodity', 'commodities', 'metal', 'energy']
    ...
    def _fetch_live_news(self, market: str = 'crypto') -> List[NewsItem]:
        ...
        news_items = []
        feeds = LIVE_NEWS_FEEDS.get(market, [])
        if not feeds:
            # Fallback only to global/stock news if market requested is unknown
            feeds = LIVE_NEWS_FEEDS.get('stocks', []) 
