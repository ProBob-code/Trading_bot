...
class BinanceCryptoProvider:
    ...
    def _safe_get(self, url: str, params: Dict = None, timeout: int = 10) -> Optional[requests.Response]:
        """Internal helper for making resilient GET requests."""
        try:
            response = self.session.get(url, params=params, timeout=timeout)
            response.raise_for_status()
            return response
        except requests.exceptions.ConnectionError as ce:
            if "NameResolutionError" in str(ce) or "[Errno -3]" in str(ce):
                logger.error(f"🌐 DNS FAILURE: Could not resolve {self.BASE_URL}. Check internet/DNS settings.")
            else:
                logger.error(f"🔌 CONNECTION ERROR: Failed to connect to {self.BASE_URL}: {ce}")
        except requests.exceptions.Timeout:
            logger.error(f"🕒 TIMEOUT: Request to {url} timed out after {timeout}s")
        except requests.exceptions.HTTPError as he:
            logger.error(f"❌ HTTP ERROR {he.response.status_code}: {url}")
    ...
    def get_current_price(self, symbol: str = "BTCUSDT") -> Dict:
        ...
        response = self._safe_get(url, params={"symbol": symbol.upper()})
        ...
        return {'symbol': symbol, 'price': 0, 'status': 'error'}
